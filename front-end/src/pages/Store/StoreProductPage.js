...
function StoreProductPage() {
  ...
  async function removeProduct() {
    try {
      const signer = provider.getSigner();
      const storeContract = new ethers.Contract(store, StoreContract.abi, signer);

      const tx = await storeContract.removeProduct(Number(id));
      await tx.wait();

      alert("Product removed successfully!");
      navigate(`/store/${store}`); // 👈 redirect back to the store page
    } catch (error) {
      console.error(error);
