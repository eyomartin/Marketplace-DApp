...
function ProductPage() {
  ...
  const loadStoredFeedback = () => {
    const key = `feedback-${id}-${data.account}`;
    const storedRating = localStorage.getItem(key);
    if (storedRating) setRating(parseInt(storedRating));
  };
  ...
  const getBuyerFeedback = () => {
    const key = `feedback-${id}-${productState.buyer}`;
  ...
  const updateBalance = async () => {
    ...
    dispatch(updateAccountData({ ...data, balance: utils.formatUnits(balance) }));
  };
  ...
  const sendTip = async () => {
    if (!tipAmountUSD || isNaN(tipAmountUSD) || Number(tipAmountUSD) <= 0) {
      alert("Please enter a valid tip amount in USD!");
      return;
    }
    ...
    try {
      setSendingTip(true);
      const signer = provider.getSigner();
      const price_in_eth = await convertPrice(tipAmountUSD);
  
      const tx = await signer.sendTransaction({
        to: productState.seller,
        value: price_in_eth,
      });
      await tx.wait();
  
      alert(`Tip of ${tipAmountUSD}$ successfully sent! ❤️`);
  
      const tipKey = `tip-${id}-${productState.seller}`;
      localStorage.setItem(tipKey, tipAmountUSD);
          ...
          <div className="p-3">
            {Number(productState.buy_price_in_ETH) === 0 ? (
              ...
              <div>
                ...
                <b>{parseFloat(productState.buy_price_in_ETH).toFixed(4)} ETH</b>
              </div>
            )}
  ...
  <>
    ...
    {(() => {
      ...
      if (tipAmount) {
        return <p>You have been tipped {tipAmount} USD ❤️</p>;
      }
      return null;
    })()}
  </>
              ...
              <>
                {productState.status === "IN SALE" && (
                  <Button variant="primary" onClick={() => purchase(id)}>
                    {loading ? <CircularProgress size={26} /> : "Buy"}
                  </Button>
                )}
                {productState.status === "PENDING" && (
                  <>
                ...
                {productState.status === "SOLD" && (
                  <div>
                    ...
                    {rating ? (
                      <p>You left a feedback of {rating} ★</p>
                    ) : (
                      <div>
                        <p>Leave feedback:</p>
                        {[1, 2, 3, 4, 5].map((val) => (
                    ...
                    <div className="mt-4">
                      <Form.Group>
                        <Form.Label>Tip the Seller (USD)</Form.Label>
                        <Form.Control
                          type="number"
                          min="0"
                          placeholder="Enter amount..."
                          value={tipAmountUSD}
                          onChange={(e) => setTipAmountUSD(e.target.value)}
                        />
                      </Form.Group>
                      <Button
                        variant="success"
                        ...
                        disabled={sendingTip}
                      >
                        {sendingTip ? <CircularProgress size={26} /> : "Send Tip ❤️"}
                      </Button>
                    </div>
                  </div>
                )}
